...
describe('PaymentService', () => {
  ...
  describe('confirmPayment', () => {
    it('should mark transaction as completed', async () => {
      ...
      expect(mockStripeProvider.confirmPayment).toHaveBeenCalledWith(
        'pi_test_123',
      );
      expect(mockTransactionRepo.save).toHaveBeenCalled();
    ...
    it('should throw NotFoundException if transaction not found', async () => {
      ...
      await expect(service.confirmPayment('t-999')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('refund', () => {
    it('should mark transaction as refunded', async () => {
      await service.refund('t-123');
      expect(mockStripeProvider.refundPayment).toHaveBeenCalledWith(
        'pi_test_123',
      );
      expect(mockTransactionRepo.save).toHaveBeenCalled();
    });
