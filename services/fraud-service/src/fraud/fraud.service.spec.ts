...
describe('FraudService', () => {
  ...
  describe('analyzePriceChange', () => {
    ...
    it('should create alert on price anomaly', async () => {
      mockPriceRule.checkPriceAnomaly.mockReturnValue({
        isAnomaly: true,
        severity: 'high',
        reason: 'Too expensive',
      });
      mockPriceRule.checkRapidPriceChange.mockReturnValue({
        isAnomaly: false,
        severity: 'none',
        reason: '',
      });
    ...
    it('should create alert on rapid price change', async () => {
      ...
      await service.analyzePriceChange(data);
      expect(mockAlertRepo.create).toHaveBeenCalled();
    });
  ...
  describe('findAll', () => {
    ...
    it('should filter by resolved status', async () => {
      ...
      expect(mockAlertRepo.find).toHaveBeenCalledWith(
        expect.objectContaining({ where: { isResolved: true } }),
      );
    });
  });
  ...
  describe('resolve', () => {
    it('should resolve an alert', async () => {
      ...
      await service.resolve('a-1', 'admin-1');
