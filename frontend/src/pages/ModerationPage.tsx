...
const ModerationPage: React.FC = () => {
  ...
  const handleAction = async (id: string, status: 'approved' | 'rejected') => {
    ...
    try {
      ...
      setError(
        `Erreur lors de ${status === 'approved' ? "l'approbation" : 'du rejet'}`,
      );
    } finally {
      ...
      >
        ...
        >
          <div>
            ...
            <p style={{ color: 'var(--color-text-muted)' }}>
              {products.length} article{products.length === 1 ? '' : 's'} en
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {products.map((product) => (
              ...
              >
                ...
                >
                  <p
                    style={{
                      ...
                      margin: 0,
                      overflow:
                        expandedId === product.id ? 'visible' : 'hidden',
                      textOverflow: 'ellipsis',
                      whiteSpace:
                        expandedId === product.id ? 'normal' : 'nowrap',
                ...
                >
                  <span>
                    🆔 Vendeur : <code>{product.sellerId.slice(0, 8)}...</code>
                  </span>
                  {product.shippingCost > 0 && (
                    <span>🚚 Frais : {formatPrice(product.shippingCost)}</span>
                  )}
                  <span>
                    💸 Commission : {formatPrice(product.price * 0.05)}
                  </span>
                </div>
